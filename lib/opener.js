...
export function revealInFolder(dir) {
  ...
  if (platform === 'darwin') {
    return spawnDetached('open', [absoluteDir], `could not reveal folder: ${absoluteDir}`);
  }

  if (platform === 'win32') {
    return spawnDetached('explorer', [absoluteDir], `could not reveal folder: ${absoluteDir}`);
  }

  return spawnDetached('xdg-open', [absoluteDir], `could not reveal folder: ${absoluteDir}`);
}
...
export function openInBrowser(url) {
  ...
  if (platform === 'win32') {
    return spawnDetached('cmd', ['/c', 'start', '""', url], `could not open browser: ${url}`);
  }
